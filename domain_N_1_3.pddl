; EXAMPLE:  go from "4" to "7", from "1" to "5" and from "2" to "3" and "6" when both cars are stationed at "0" with refill.
...
(define (domain bus)
    ...
    (:action drive_4_1
        ...
        :effect (and
            (not (location ?c p4))
            (itinerary ?c p4 p1)
            (assign (car-distance ?c p1)(distance p4 p1))
            (assign (speed ?c) (ave_speed))
        )
    )
    (:event arrived_from_p4_to_p1
        :parameters (?c -car)
        :precondition (and
            (<= (car-distance ?c p1) 0)
            (itinerary ?c p4 p1)
    ...
    (:action drive_4_5
        ...
        :effect (and
            (not (location ?c p4))
            (itinerary ?c p4 p5)
            (assign (car-distance ?c p5)(distance p4 p5))
            (assign (speed ?c) (ave_speed))
        )
    )
    (:event arrived_from_p4_to_p5
        :parameters (?c -car)
        :precondition (and
            (<= (car-distance ?c p5) 0)
            (itinerary ?c p4 p5)
    ...
    (:action drive_5_7
        ...
        :effect (and
            (not (location ?c p5))
            (itinerary ?c p5 p7)
            (assign (car-distance ?c p7)(distance p5 p7))
            (assign (speed ?c) (ave_speed))
        )
    )

    (:event arrived_from_p5_to_p7
        :parameters (?c -car)
        :precondition (and
            (<= (car-distance ?c p7) 0)
            (itinerary ?c p5 p7)
    ...
    (:action boarding_people_p4
        ...
        :precondition (and
            ...
            (> (people_requested_itinerary p4 p7) 0)
            (<= (people_in_car ?c) 30)
        )
        :effect (and
            (decrease
                (people_at_station p4)
                (people_requested_itinerary p4 p7)
                )
            (increase
                (people_in_car ?c)
                (people_requested_itinerary p4 p7)
